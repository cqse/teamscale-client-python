...
class TeamscaleClient:
    ...
    def upload_coverage_data(
            self, coverage_files: List[str], coverage_format: CoverageFormats,
            timestamp_or_revision: Union[datetime.datetime, str],
            message: str, partition: str, repository: Optional[str] = None
    ) -> requests.Response:
        ...
        return self.upload_report(
            coverage_files, coverage_format, timestamp_or_revision, message, partition, repository=repository
        )

    def upload_report(
            self, report_files: List[str], report_format: Union[ReportFormats, CoverageFormats, UnitTestReportFormats],
            timestamp_or_revision: Union[datetime.datetime, str], message: str, partition: str,
            move_to_last_commit: bool = True, repository: Optional[str] = None
    ) -> requests.Response:
        ...
        if isinstance(timestamp_or_revision, datetime.datetime):
            parameter["t"] = self._get_timestamp_parameter(timestamp_or_revision)
        elif isinstance(timestamp_or_revision, str):
            parameter["revision"] = timestamp_or_revision
            if repository:
                parameter["repository"] = repository
        else:
            raise ServiceError("Unknown format for timestamp/ revision parameter!")
